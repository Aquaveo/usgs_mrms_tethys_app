"""Database-backed persistence for background flood-alert jobs.

All reads/writes go through the app ``jobs_db`` persistent store so every portal
replica sees the same records. Methods return plain dicts, never ORM objects.
Mirrors fimserve_viewer's job_store.
"""
...
class JobStore:
    """Reads and writes flood-alert job records in the ``jobs_db`` store."""
    ...
    def create_or_get_active(self, state: str, run_id: str, key: str, params: dict, owner: str) -> Tuple[dict, bool]:
        ...
        try:
            with self.session() as session:
                job = Job(key=key, state=state, run_id=run_id, params=params, claimed_by=owner)
                session.add(job)
                session.flush()
    ...
    def claim(self, job_id: str, worker: str) -> bool:
        ...
        with self.session() as session:
            claimed = (
                ...
                .update({"status": JobStatus.RUNNING, "heartbeat_at": utcnow(), "updated_at": utcnow()})
            )
            return claimed == 1

    def update_progress(self, job_id: str, message: str = "") -> None:
        """Record a progress message from the running pipeline (stays RUNNING)."""
        self.update_fields(job_id, status=JobStatus.RUNNING, message=message, heartbeat_at=utcnow())

    def finish(self, job_id: str, status: str, message: str, result_key: str = "") -> None:
        """Record the terminal state of a job."""
        self.update_fields(job_id, status=status, message=message, result_key=result_key)
    ...
    def find_active_for_state(self, state: str) -> Optional[dict]:
        """Return the active job for a state, or None."""
        return self.first_active_dict(Job.state == state)
